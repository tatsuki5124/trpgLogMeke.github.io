...
import { isStatusTargetName } from '@/logmake/systems/coc/shared'
...
describe('parseCoc7DiceToken', () => {
  ...
  it('parses a D100 success result', () => {
    ...
    expect(result?.dice.primaryRoll).toBe(17)
    expect(result?.dice.resultText).toBe('成功')
    expect(result?.highlight).toBe('success')
    expect(result?.dice.targets).toHaveLength(1)
    expect(result?.dice.targets[0].name).toBe('聞き耳')
  })
  ...
  it('parses a D100 hard success result', () => {
    ...
    expect(result?.dice.resultText).toBe('ハード成功')
    expect(result?.highlight).toBe('success')
    expect(result?.dice.meta).toEqual({ cocOption: 'h' })
  })
  ...
  it('parses CoC7 bonus dice shorthand without parentheses', () => {
    ...
    expect(result?.dice.command).toBe('CC1&lt;=50')
    expect(result?.dice.targets[0]?.name).toBe('目星')
    expect(result?.dice.primaryRoll).toBe(5)
    expect(result?.dice.resultText).toBe('イクストリーム成功')
  })
  ...
  it('parses a D100 extreme success result', () => {
    ...
    expect(result?.dice.resultText).toBe('イクストリーム成功')
    expect(result?.dice.meta).toEqual({ cocOption: 'e' })
  })
  ...
  it('parses a critical success', () => {
    ...
    expect(result?.dice.resultText).toBe('クリティカル')
    expect(result?.highlight).toBe('success')
  })
  ...
  it('parses a fumble result', () => {
    ...
    expect(result?.dice.resultText).toBe('ファンブル')
    expect(result?.highlight).toBe('failure')
    expect(result?.dice.primaryRoll).toBe(100)
  })
  ...
  it('parses a failure result', () => {
    ...
    expect(result?.dice.resultText).toBe('失敗')
    expect(result?.highlight).toBe('failure')
  })
  ...
  it('does not apply skill aliases (CoC7 has no aliases)', () => {
    ...
    expect(result?.dice.targets[0]?.name).toBe('ma')
  })
...
describe('isStatusTargetName', () => {
  it('returns true for plain stat name (exact match)', () => {
    expect(isStatusTargetName('STR')).toBe(true)
  })

  it.each(['STR*5', 'STR * 4', 'STR×3'])(
    'returns true for stat multiplier expression (%s)',
    (targetName) => {
      expect(isStatusTargetName(targetName)).toBe(true)
    }
  )

  it.each(['STR+DEX', 'STR + DEX'])(
    'returns true for stat addition expression (%s)',
    (targetName) => {
      expect(isStatusTargetName(targetName)).toBe(true)
    }
  )

  it('returns false for a Japanese label containing a stat abbreviation', () => {
    expect(isStatusTargetName('筋力(STR)')).toBe(false)
  })

  it.each(['STRATEGY', 'STR技能'])(
    'returns false for non-status target (%s)',
    (targetName) => {
      expect(isStatusTargetName(targetName)).toBe(false)
    }
  )
})
